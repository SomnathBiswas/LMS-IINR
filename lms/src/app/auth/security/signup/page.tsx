...
export default function SecuritySignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/auth/security/register', {
        method: 'POST',
        headers: {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-b from-blue-50 to-indigo-100 p-4">
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1">
          <CardTitle className="text-2xl font-bold text-center text-[#59159d]">Security Sign Up</CardTitle>
        ...
        <CardFooter className="flex flex-col space-y-2">
          <p className="text-sm text-center text-gray-500">
            ...
            <Link href="/auth/security/login" className="text-[#59159d] hover:underline">
              Sign in
            </Link>
          </p>
