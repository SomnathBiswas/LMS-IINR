...
export async function POST(req: Request) {
  try {
    const { accountsId, password } = await req.json();
    const startTime = Date.now();

    // Validate the input
    if (!accountsId || !password) {
      return NextResponse.json(
        { error: 'Accounts ID and password are required' },
        { status: 400 }
      );
    }
    ...
    console.log(`Accounts login attempt: ${accountsId}, found: ${!!user}, time: ${Date.now() - startTime}ms`);
    ...
    const redirectUrl = '/accounts-dashboard';
    ...
    response.cookies.set('user_role', 'Accounts', {
      httpOnly: false,
      secure: process.env.NODE_ENV === 'production',
      sameSite: 'lax',
      path: '/',
    ...
    response.headers.set('X-Auth-Role', 'Accounts');
